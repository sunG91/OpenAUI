 ...
 * @param {function} options.testModel - 模型调用函数 (payload) => Promise<{ content, reasoning_content }>
...
export async function decompose(options) {
  const {
    ...
    enableThinking = true,
    testModel,
    outputMode = 'display', // 'display' | 'file'
  } = options;

  let thinkingOutput = '';
  if (enableThinking && testModel) {
    const thinkingMsg = `用户目标：${userGoal}\n\n${THINKING_PROMPT}`;
    const thinkingRes = await testModel({ vendorId, modelId, message: thinkingMsg });
    thinkingOutput = (thinkingRes?.content ?? '').trim();
  }

  const outputHint = options?.outputMode === 'display'
...
${thinkingOutput ? `【模型深度思考】\n${thinkingOutput}\n\n基于以上思考，` : ''}请将目标拆分为可执行的步骤计划。\n\n${PLAN_SCHEMA}`;
  ...
  const res = await testModel({ vendorId, modelId, message: decomposeMsg });
