...
- 天枢架构：**天枢测试（AUI 画布工作台）** 已落地——三层决策（天枢殿 / 紫微台 / 璇玑府）推演流、七大执行部 React Flow 可视化、本机 Shell 执行、任务总结报告、赏罚台评分与模块化 skills 沉淀、赏罚账本编辑等；当前处于**功能测试中**，持续迭代体验与提示词。
- 文档：README 与《产品设计文档》更新天枢状态说明；新增 **v0.2.1 天枢架构测试工作台** 参考图（`images/mdimg/天枢架构测试图0.2.1.png`）。
- 版本号：应用与 npm 包版本 **0.2.1**。

## 0.2.0（2026-03-19）

- 新增：**MCP 对接 Chat** 主聊天支持 MCP 工具调用，启用 MCP 后模型可自动调用已配置工具；检索结果以精华摘要形式传给模型，节省 token；MCP 标签固定于回复底部，可点击查看原始返回与 AI 总结。
- 优化：**MCP 总结** 单次工具返回提取精华（4–8 句），覆盖业务地位、核心数据、发展前景等；标签展示限制 2–7 字；支持 `[工具名]` 与 `【标签名称】` 两种格式识别为可点击标签。
- 优化：**流式过渡** 正在检索/正在生成回答与正文流式输出同区域平滑替换；标签始终在底部，不再跳转。
- 修复：**生成中断兜底** 当模型持续返回 tool_calls 导致无文本时，自动用 MCP 精华汇总拼接展示，避免只显示「生成被中断」；runChat 异常时向前端发送错误信息。
- 文档：产品设计文档新增 4.5 MCP 对接 Chat 参考图（v0.2.0）；README 界面预览更新为 v0.2.0。
...
- 新增：**天枢架构** 三层决策中枢（天枢殿、紫微台、璇玑府）+ 七大执行部（启元部、文翰部、玄枢部、金匮部、监正部、天机部、外交部）+ 赏罚台机制；架构配置持久化到系统配置。
- 新增：**架构图** 无连线、主次分明的架构图风格，机制以虚线框包裹整体；支持流程图/文档介绍切换。
